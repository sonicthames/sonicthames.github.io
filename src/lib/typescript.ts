...
export type LeafPathTree<T> = {
  readonly [P in keyof T]-?: T[P] extends object ? readonly [P, ...LeafPath<T[P]>] : readonly [P];
};
export type LeafPath<T> = LeafPathTree<T>[keyof LeafPathTree<T>];

export type JoinTuple<S extends string, KS extends readonly string[]> = KS extends readonly []
  ? ""
