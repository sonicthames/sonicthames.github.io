...
import type { HasDateTimeOption, HasIntervalOption, SoundBase } from "./domain/base";
...
const ensureIsoValid =
  ...
  (value: A): E.Either<D.DecodeError, A> =>
    value.invalidReason === null ? E.right(value) : E.left(isoError(label, input)(value));
  ...
  pipe(
    ...
    D.parse((value) =>
      pipe(
        value,
        O.fromNullable,
        O.traverse(E.Applicative)((input) => pipe(parser(input), ensureIsoValid(label, input))),
      ),
    ),
  );

const optional = <A>(decoder: D.Decoder<unknown, A>): D.Decoder<unknown, O.Option<A>> =>
  pipe(D.nullable(decoder), D.map(O.fromNullable));

const readonlyArray = <A>(decoder: D.Decoder<unknown, A>) => pipe(D.array(decoder), D.readonly);

const readonlyStruct = <A>(decoder: D.Decoder<unknown, A>) => pipe(decoder, D.readonly);

export const D_Category = D.union(D.literal("Listen"), D.literal("See"), D.literal("Feel"));
...
export const D_Sound = pipe(D_SoundBase, D.intersect(D.union(D_DateTime, D_Interval)));
