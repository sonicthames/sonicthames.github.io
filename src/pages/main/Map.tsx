...
import { type Category, type Sound, showDateTime, showInterval } from "../../domain/base";
...
const EnvDecoder = D.struct({
  VITE_MAPBOX_TOKEN: pipe(
    D.string,
    D.refine((value): value is string => value.trim().length > 0, "Mapbox token"),
  ),
});
...
const lngLatBounds = new mapboxgl.LngLatBounds(
  new mapboxgl.LngLat(center.lng - LNG_BOUND_OFFSET, center.lat - LAT_BOUND_OFFSET),
  new mapboxgl.LngLat(center.lng + LNG_BOUND_OFFSET, center.lat + LAT_BOUND_OFFSET)
);
      ...
      <header className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-4">
          <fieldset className="flex gap-1 border-none p-0 m-0">
            ...
            >
              <Icon name="Listen" width={headerIconSize} height={headerIconSize} />
            </button>
            <button
              type="button"
              onClick={() => {
...
export const MainMap = ({ history, sounds }: Props): JSX.Element => {
  ...
  useEffect(
    () =>
      pipe(
        goTo$,
        ($) =>
          $.subscribe(({ latitude, longitude, zoom, transitionDuration, transitionEasing }) => {
  ...
  useEffect(() =>
    pipe(hoverClose$, ($) => $.subscribe(() => setHoverSoundO(O.none)), lazyUnsubscribe),
  );

  const [expand$] = useState(() => new BehaviorSubject<boolean>(O.isSome(soundO)));
